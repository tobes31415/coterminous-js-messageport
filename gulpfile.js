...
gulp.task('copyWorker', function(){
    return gulp.src(["./worker.js"]).pipe(
    gulp.dest("./bin"))
});

gulp.task('copyCoterminous',function(){
    return gulp.src(["./bower_components/coterminous-js/bower/coterminous.js"]).pipe(
    gulp.dest("./bin"))
})
...
gulp.task('webpack-devA', function(callback){
    packFiles('./main','./bin', 'main.js', true, callback);
});
gulp.task('webpack-devB', function(callback){
    packFiles('./publish','./bin', 'messageport.js', false, callback);
})

gulp.task('build-npm', function(callback){
    packFiles('./publish','./npm', 'messageport.js', false, callback);
})
gulp.task('build-bower', function(callback){
    packFiles('./publish','./bower', 'messageport.js', false, callback);
})

gulp.task('build', function(done){
    runSequence('webpack-devA','webpack-devB', 'copyIndex', 'copyWorker','copyCoterminous', done);
});
